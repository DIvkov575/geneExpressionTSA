...
def nbeats_forecast(model, history, steps=1):
    ...
    try:
        ...
        if 'NBEATS' in forecast.columns:
            predictions = forecast['NBEATS'].values
...
def walk_forward_nbeats_forecast(data, model, min_history=30):
    """
    Perform walk-forward forecasting on a time series using N-BEATS.
    ...
    for i in range(min_history, len(data)):
        ...
        try:
            forecast_values = nbeats_forecast(model, synthetic_history, steps=1)
...
def main():
    print("Walk-Forward N-BEATS Forecasting for CRE.csv Column '1'")
    print("=" * 60)
    ...
    forecasts, actuals, errors, successful_forecasts = walk_forward_nbeats_forecast(
    ...
    print()
    print("Walk-Forward N-BEATS Forecasting Results:")
    print("=" * 50)
    print(f"Total forecasts made: {len(forecasts)}")
    print(f"Successful N-BEATS forecasts: {successful_forecasts}")
    ...
    output_path = 'nbeats_walk_forward_results.csv'
