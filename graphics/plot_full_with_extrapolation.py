#!/usr/bin/env python3
"""
Plot full time series with model extrapolations from step 500.
Shows original CRE.csv data for steps 0-499 and forecasts from 500+.
"""
...
def load_original_data():
    ...
    try:
        ...
        full_data = df['2'].values
        
        # Split into before and after 500
        data_before_500 = full_data[:500]
        data_after_500 = full_data[500:]
        
        steps_before_500 = np.arange(len(data_before_500))
        steps_after_500 = np.arange(500, 500 + len(data_after_500))
        
        return steps_before_500, data_before_500, steps_after_500, data_after_500
...
def create_full_with_extrapolation_plot(all_data, failed_models):
    """Create the full time series plot with extrapolations from step 500."""
    ...
    # Load original data 
    print("Loading original CRE.csv data...")
    steps_before_500, data_before_500, steps_after_500, data_after_500 = load_original_data()
    
    if steps_before_500 is None:
        print("❌ Cannot plot without original data!")
        return
    ...
    # Plot original data for first 500 steps
    plt.plot(steps_before_500, data_before_500, 
            color=colors.get('actual', '#000000'),
            linestyle=line_styles.get('actual', '-'),
            linewidth=2.5, 
            label='Actual Data (0-499)', 
            alpha=0.8)
    
    # Plot actual data for 500+ steps
    if len(data_after_500) > 0:
        plt.plot(steps_after_500, data_after_500, 
                color='#000000',  # Same black color
                linestyle='-',
                linewidth=2.5, 
                label='Actual Data (500+)', 
                alpha=0.8)
    ...
    # Add vertical line at step 500 to show where extrapolation begins
    plt.axvline(x=500, color='red', linestyle=':', alpha=0.7, linewidth=2,
               label='Extrapolation Start (Step 500)')
    
    # Customize plot
    plt.xlabel('Time Step', fontsize=12)
    plt.ylabel('Column 2 Value', fontsize=12) 
    plt.title('Full Time Series with 500+ Extrapolation Forecasts', fontsize=14, fontweight='bold')
    ...
    plot_filename = f'graphics/full_with_extrapolation_{timestamp}.png'
...
def main():
    ...
    print("="*60)
    print("FULL TIME SERIES WITH EXTRAPOLATION PLOTTER")
    print("="*60)
    ...
    # Create plot
    print("\n3. Creating full time series plot with extrapolations...")
    create_full_with_extrapolation_plot(all_data, failed_models)
