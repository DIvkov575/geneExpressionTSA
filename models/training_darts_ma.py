...
def train_and_evaluate_moving_average():
    """
    Trains and evaluates a MovingAverage model on the CRE dataset.
    """
    # --- 1. Setup Paths ---
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    data_path = os.path.join(project_root, 'data', 'CRE.csv')
    model_dir = os.path.join(script_dir, 'darts_logs', 'moving_average_cre_model_baseline')
    ...
    # MovingAverage model requires a window parameter
    # Let's use a window of 12, similar to seasonal_periods
    model = NaiveMovingAverage(window=12)

    total_mae = 0
    num_series_evaluated = 0
    
    print(f"--- Evaluating MovingAverage model (window=12) ---")

    for i, train_series in enumerate(train_series_list):
        val_series = val_series_list[i]
        
        try:
            # MovingAverage model needs to be 'fit' on the series it is predicting from
    ...
    print("\n--- Baseline MovingAverage training complete ---")


if __name__ == '__main__':
    train_and_evaluate_moving_average()
