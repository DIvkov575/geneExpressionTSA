...
def train_and_predict_theta(X_train, y_train, X_test, y_test):
    """
    Trains a Theta model and returns predictions.
    """
...
if __name__ == "__main__":
    ...
    # Train and predict using Theta model
    test_theta, predictions_theta = train_and_predict_theta(X_train, y_train, X_test, y_test)
    rmse_theta = np.sqrt(mean_squared_error(test_theta.to_dataframe(), predictions_theta.to_dataframe()))

    print(f'Theta Average RMSE: {rmse_theta}')
