...
class MultiHorizonARIMA_v3:
    ...
    1. Full ARIMA(p,d,q) with MA components via Maximum Likelihood Estimation
    2. Multi-series training (shared parameters across multiple series)
    3. Efficient multi-step ahead forecasting
    
    Key improvements over v2:
    - Supports MA (Moving Average) components
    - Uses MLE for parameter estimation
    - Can recover true ARIMA parameters
    - Suitable for both forecasting and parameter identification
    ...
    def __init__(self, p=1, d=0, q=0):
        ...
        self.d = d
        self.q = q
        self.params_ = None  # θ = [c, φ_1,...,φ_p, θ_1,...,θ_q, log(σ²)]
        self.fitted_ = False
    ...
    def fit(self, series_list, init_params=None, maxiter=1000, method='L-BFGS-B'):
        """
        ...
        method : str
            Optimization method ('L-BFGS-B', 'BFGS', 'Nelder-Mead', etc.)
            
        Returns:
        --------
        self : MultiHorizonARIMA_v3
            Fitted model
        """
    ...
    def summary(self):
        ...
        print(f"Multi-Horizon ARIMA({self.p},{self.d},{self.q}) v3 Model")
        print("=" * 60)
    ...
    def predict_in_sample(self, series):
        """
        Generate in-sample predictions (fitted values) for a given series.
        ...
        --------
        predictions : ndarray
            In-sample predictions (same length as input series)
        """
        if not self.fitted_:
            raise ValueError("Model must be fitted first")
        ...
        for t in range(len(z)):
            ...
            z_pred[t] = c + ar_term + ma_term
        
        # Inverse difference to get original scale
        predictions_original = self.inverse_difference(z_pred, history)
