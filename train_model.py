...
def train_val_test_split(X, y, series_ids, val_ratio=0.1, test_ratio=0.1):
    """
    Splits sliding window dataset into train/val/test preserving temporal order per series.
    
    Parameters:
        X, y: np.ndarray of sliding windows
        series_ids: np.ndarray mapping each sample to its series
        val_ratio: fraction of time for validation
